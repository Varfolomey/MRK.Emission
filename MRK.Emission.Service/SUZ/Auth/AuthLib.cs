    ...
    public class AuthLib
    {
        ...
        public async Task<AuthData> RegAsync(string queryString)
        {
                ...
                if (response != null)
                {
                    try
                    {
                        using (var responseStream = await response.Content.ReadAsStreamAsync())
                        {
                            ...
                            authData = JsonConvert.DeserializeObject<AuthData>(responseData);
                        }
                    }
                    catch(Exception ex)
                    {
                        return new AuthData
                        {
                            error_message = $"Unable to parse data: {responseData}"
                        };
                    }
                }
            }
            catch(InvalidOperationException)
            {
                return new AuthData
                {
                    error_message = "Parameter \"queryString\" is not a valid URI."
                };
            }
